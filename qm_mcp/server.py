...
contract the brief asked for:
    ...
    qm_query(question, k, distance_threshold)
                                natural-language query (grounded answer + sources)
...
async def qm_query(
    ...
    Set env var ``QM_USE_LLAMA_INDEX=true`` to use BAAI/bge-base-en-v1.5
    local embeddings instead of OpenAI (same response shape, no API cost).

    Args:
        question: Natural-language question to answer from the corpus.
        k: Maximum candidates before distance-threshold filtering (default 6).
        distance_threshold: Max cosine distance for inclusion (default 0.7).
    """
